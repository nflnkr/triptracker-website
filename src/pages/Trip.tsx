...
export default function TripPage() {
    ...
    useInterval(function resizeProgressSlider() {
        if (!chartRef.current || !chartBoxRef.current) return;

        chartBoxRef.current.style.width = `${chartRef.current.chartArea.width}px`;
        chartBoxRef.current.style.marginLeft = `${chartRef.current.chartArea.left}px`;
    }, 1000);
            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
                ...
                <Box sx={{ display: "flex", gap: 2, flexDirection: isPortraitMode ? "column" : "row" }}>
                    <Box sx={{ display: "flex", flexDirection: "column", gap: 1, minWidth: "19em", flexGrow: 1, order: isPortraitMode ? 2 : 1 }}>
                        <Paper elevation={3} sx={{ height: 480 }}>
                            <OlMap
                                tripId={tripId}
                                tracks={trip.tracks}
